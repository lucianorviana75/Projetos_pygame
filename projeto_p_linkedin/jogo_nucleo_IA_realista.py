...
def criar_explorador():
    return{"angulo": 0.0, "camada": 0}    

#F03
def criar_inimigo():
    return{"angulo":math.pi, "camada": 0}

#F04
def atualizar_explorador(exp, camadas, inim):
    if exp["camada"] >= len(camadas):
        return

    dif = diferenca_angular(exp["angulo"], inim["angulo"])

    PERIGO = 0.5
    VELOCIDADE_FUGA = VEL_EXPLORADOR * 2

    # 🚨 PERIGO: foge de verdade
    if abs(dif) < PERIGO:
        exp["angulo"] += VELOCIDADE_FUGA * (-1 if dif > 0 else 1)
        return

    # movimento normal
    exp["angulo"] += VEL_EXPLORADOR

    camada = camadas[exp["camada"]]
    if angulo_dentro(exp["angulo"], camada["angulo_saida"], camada["largura_saida"]):
        exp["camada"] += 1
...
def atualizar_inimigo(inim, exp, camadas):
    # nunca entra no núcleo
    inim["camada"] = min(exp["camada"], len(camadas) - 1)

    dif = diferenca_angular(inim["angulo"], exp["angulo"])

    velocidade = VEL_EXPLORADOR  # mesma velocidade

    if abs(dif) > velocidade:
        inim["angulo"] += velocidade * (1 if dif > 0 else -1)
...
def colidiu(a, b):
    return (
        a["camada"] == b["camada"]
        and abs(diferenca_angular(a["angulo"], b["angulo"])) < 0.15
    )
...
def desenhar_bolinha(entidade, cor, camadas):
    if entidade["camada"] >= len(camadas):
        r = RAIO_INICIAL // 2
    else:
        camada = camadas[entidade["camada"]]
...
def jogo():
    ...
    while True:
        ...
        if  estado == "rodando":
            atualizar_explorador(explorador, camadas, inimigo)
