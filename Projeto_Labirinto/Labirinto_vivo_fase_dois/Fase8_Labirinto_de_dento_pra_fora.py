...
while rodando:
    ...
    for i in range(quantidade_raios):
        ...
        desvio = math.sin(tempo * 0.6 + i) * 0.25
        #Agora ângulo orgânico
        angulo = (2 * math.pi / quantidade_raios) * i + desvio
        
       
        
        ritmo = math.sin(tempo* 1.2 + i)
        
        #Define ate onde o caminho
        if ritmo > 0.3:
            limite = aneis[-1]
        elif ritmo > -0.2:
            limite = aneis[2]
        ...
        x_fin = CENTRO[0] + limite * math.cos(angulo)
        y_fin = CENTRO[1] + limite * math.sin(angulo)           
        pygame.draw.line(
            tela,
            (180, 180, 180),
            CENTRO,
